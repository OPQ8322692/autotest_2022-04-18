...
form_data_list_masterapp = [(YamlReader(login_pre_file).data()["data_masterapp"])]
...
base_url_masterapp = (YamlReader(login_pre_file).data()["url_masterapp"])
...
class Test_user_login():
    ...
    def test_login_userapp(self, form_data):
        ...
        #print(type(cookies_userapp))
        return cookies_userapp

    # 师傅app登录
    @pytest.mark.parametrize("form_data", form_data_list_masterapp)
    def test_login_masterapp(self, form_data):
        res = requests.post(url=base_url_masterapp, json=form_data)
        # code = res.status_code
        # 取出cookies,然后以json格式输出
        cookie_jar = res.cookies
        # print(cookie_jar)
        #cookies_masterapp = requests.utils.dict_from_cookiejar(cookie_jar)
        print(res.text)
        #要把响应体转为字典后再取值
        response = json.loads(res.text)
        # 从响应体中提取signature（类似于token值，后面用于其它接口的调用）
        signature = response["data"]["signature"]
        signature_master_cookies = response["data"]
        #print('师傅app登录返回signature是：',signature)
        print('师傅app登录返回cookies是：', signature_master_cookies)
        dict_signature_master_cookies =dict()
        dict_signature_master_cookies["signature"] = signature_master_cookies["signature"]
        print("存储师傅signature新字典打印值为",dict_signature_master_cookies)
        #print(type(signature_master_cookies))

        # # 将signature写进全局变量globalvar.py文件里面
        # glovar = GloblVar()
        # glovar.set_value('signature_master', signature)
        # # 打印cookies值
        # signature_master = glovar.get_value("signature_master")
        # print("全局变量取到的师傅登录signaturen是：%s" %signature_master)
        # return cok
        #将结果写进配置文件里面
        with open(r'E:\zwh_autotest\config\conf_value.yml', "w") as fp:
            fp.write("signature: \"%s\"" %signature)
        return dict_signature_master_cookies
